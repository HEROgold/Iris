"""Headered rom patches by RealCritical.

These values must be applied to the .sfc version of the game. They will not work with the .smc version.
"""
from helpers.files import write_file
from helpers.headers import remove_header_offset
from logger import iris


def ac_more_enemies() -> None:
    """Increase the number of enemies in ancient cave battles."""
    iris.info("Applying RealCritical patch: ac_more_enemies (more ancient-cave battle enemies).")
    write_file.seek(remove_header_offset(0xA5B71))
    write_file.write(bytes.fromhex("17"))
    write_file.seek(remove_header_offset(0xA5B77))
    write_file.write(bytes.fromhex("15"))
    write_file.seek(remove_header_offset(0xA5B7A))
    write_file.write(bytes.fromhex("2E1B"))
    write_file.seek(remove_header_offset(0xA5B81))
    write_file.write(bytes.fromhex("271A2C"))
    write_file.seek(remove_header_offset(0xA5B8C))
    write_file.write(bytes.fromhex("2F"))
    write_file.seek(remove_header_offset(0xA5B8F))
    write_file.write(bytes.fromhex("33"))
    write_file.seek(remove_header_offset(0xA5B93))
    write_file.write(bytes.fromhex("45"))
    write_file.seek(remove_header_offset(0xA5BC1))
    write_file.write(bytes.fromhex("4A"))
    write_file.seek(remove_header_offset(0xA5B9C))
    write_file.write(bytes.fromhex("3A46"))
    write_file.seek(remove_header_offset(0xA5BA3))
    write_file.write(bytes.fromhex("49"))
    write_file.seek(remove_header_offset(0xA5BD6))
    write_file.write(bytes.fromhex("79"))
    write_file.seek(remove_header_offset(0xA5BDF))
    write_file.write(bytes.fromhex("A5"))
    write_file.seek(remove_header_offset(0xA5BE5))
    write_file.write(bytes.fromhex("A9"))
    write_file.seek(remove_header_offset(0xA5BEB))
    write_file.write(bytes.fromhex("A5"))
    write_file.seek(remove_header_offset(0xB21A1))
    write_file.write(bytes.fromhex("45002C150445"))
    write_file.seek(remove_header_offset(0xBC22D))
    write_file.write(bytes.fromhex("90"))
    write_file.seek(remove_header_offset(0xBC30D))
    write_file.write(bytes.fromhex("35"))
    write_file.seek(remove_header_offset(0xBC336))
    write_file.write(bytes.fromhex("5C"))
    write_file.seek(remove_header_offset(0xBC356))
    write_file.write(bytes.fromhex("22"))
    write_file.seek(remove_header_offset(0xBC38D))
    write_file.write(bytes.fromhex("58"))
    write_file.seek(remove_header_offset(0xBC395))
    write_file.write(bytes.fromhex("6B"))
    write_file.seek(remove_header_offset(0xBC3D5))
    write_file.write(bytes.fromhex("1F"))
    write_file.seek(remove_header_offset(0xBC415))
    write_file.write(bytes.fromhex("AB"))
    write_file.seek(remove_header_offset(0xBC41D))
    write_file.write(bytes.fromhex("57"))
    write_file.seek(remove_header_offset(0xBC43D))
    write_file.write(bytes.fromhex("0B"))
    write_file.seek(remove_header_offset(0xBC45E))
    write_file.write(bytes.fromhex("59"))
    write_file.seek(remove_header_offset(0xBC4FD))
    write_file.write(bytes.fromhex("5A56"))
    write_file.seek(remove_header_offset(0xBC51D))
    write_file.write(bytes.fromhex("5565"))
    write_file.seek(remove_header_offset(0xBC58D))
    write_file.write(bytes.fromhex("14"))
    write_file.seek(remove_header_offset(0xBC59D))
    write_file.write(bytes.fromhex("13"))
    write_file.seek(remove_header_offset(0xBC5AD))
    write_file.write(bytes.fromhex("59"))
    write_file.seek(remove_header_offset(0xBC5BD))
    write_file.write(bytes.fromhex("61"))
    write_file.seek(remove_header_offset(0xBC5DE))
    write_file.write(bytes.fromhex("62"))

def fix_cave_chest_table() -> None:
    """Fix the ancient cave chest table to match the original game."""
    iris.info("Applying RealCritical patch: fix_cave_chest_table.")
    write_file.seek(remove_header_offset(0x19376))
    write_file.write(bytes.fromhex("5C4AD8E16B"))
    write_file.seek(remove_header_offset(0xB6078))
    write_file.write(bytes.fromhex("40"))
    write_file.seek(remove_header_offset(0xB68DA))
    write_file.write(bytes.fromhex("40"))
    write_file.seek(remove_header_offset(0xB6D68))
    write_file.write(bytes.fromhex("40"))
    write_file.seek(remove_header_offset(0xB71BC))
    write_file.write(bytes.fromhex("40"))
    write_file.seek(remove_header_offset(0xB74CF))
    write_file.write(bytes.fromhex("20"))
    write_file.seek(remove_header_offset(0xB74ED))
    write_file.write(bytes.fromhex("20"))
    write_file.seek(remove_header_offset(0xB7699))
    write_file.write(bytes.fromhex("40"))
    write_file.seek(remove_header_offset(0xB7BE2))
    write_file.write(bytes.fromhex("20"))
    write_file.seek(remove_header_offset(0xB7BEF))
    write_file.write(bytes.fromhex("20"))

    write_file.seek(remove_header_offset(0x30DA20))
    write_file.write(bytes.fromhex("E2 20 89 01 08 4A 8F 1C 21 00 22 C7 82 80 48 8F 1B 21 00 7B 8F 1B 21 00 68 28 D0 02 A9 00 C2 20 18 6F 34 21 00 6F 34 21 00 60 A2 00 10 86 54 C2 20 A5 60 29 FF 01 4A 20 20 D8 EB 29 FF 00 0A 18 65 54 AA 5C 88 91 83"))

def fix_menu() -> None:
    """Fix the menu to avoid softlocks."""
    iris.info("Applying RealCritical patch: fix_menu.")
    write_file.seek(remove_header_offset(0xDA0))
    write_file.write(bytes.fromhex("4C1AFC"))
    write_file.seek(remove_header_offset(0x7E1A))
    write_file.write(bytes.fromhex("8E 75 05 5A A0 0B 00 A9 20 9D 40 00 9E 41 00 E8 E8 88 10 F5 7A 4C A3 8B"))

def gift_mode() -> None:
    """Enable gift mode, basically NG+."""
    iris.info("Applying RealCritical patch: gift_mode (NG+).")
    write_file.seek(remove_header_offset(0x16B9E))
    write_file.write(bytes.fromhex("A902"))

def killer_names() -> None:
    """Change the names of the killers in the game to "Killer"."""
    # FIXME: This is a placeholder. This should be done with Iris.
    iris.info("Applying RealCritical patch: killer_names.")
    write_file.seek(remove_header_offset(0xB3105))
    write_file.write(bytes.fromhex("4B 69 6C 6C 65 72"))
    write_file.seek(remove_header_offset(0xB3152))
    write_file.write(bytes.fromhex("4B 69 6C 6C 65 72"))
    write_file.seek(remove_header_offset(0xF4DF0))
    write_file.write(bytes.fromhex("4B 69 6C 6C 65 72"))
    write_file.seek(remove_header_offset(0xF4DFC))
    write_file.write(bytes.fromhex("4B 69 6C 6C 65 72"))
    write_file.seek(remove_header_offset(0xF53C0))
    write_file.write(bytes.fromhex("4B 69 6C 6C 65 72"))
    write_file.seek(remove_header_offset(0xF53CC))
    write_file.write(bytes.fromhex("4B 69 6C 6C 65 72"))

def jelly_damage_display() -> None:
    iris.info("Applying RealCritical patch: jelly_damage_display.")
    write_file.seek(remove_header_offset(0xB097E))
    write_file.write(bytes.fromhex("C748"))
    write_file.seek(remove_header_offset(0xB5087))
    write_file.write(bytes.fromhex("4D 61 73 74 65 72 20 20 20 20 20 20 20 32 32 86 00 FC 26 00 00 0A 00 01 00 FF 01 01 01 01 00 01 00 07 28 00 08 DA 00 00 32 01 32 05 07 87 00 00 42 00 24 00 E7 03 01 0C 87 01 00 0C 23 05 00 1E 86 00 07 87 01 00 58 00 24 01 E7 03 01 0C 87 02 00 0C 23 05 00 1E 87 00 07 87 02 00 66 00 1E 89 41 0C 87 03 00 00 32 06 13 80 00 0F 80 02 00 1A 80 24 00 80 80 01 1E 88 0C 23 05 00 5B 00 5B 00"))
