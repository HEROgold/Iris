...
from ctypes import POINTER, _Pointer, c_bool, c_char_p, c_double, c_int
...
class errordata(ctypes.Structure):
    ...
    def __repr__(self) -> str:
        return f"<asar error: {self.fullerrdata.decode()!r}>"


# for internal use only. getalllabels() returns a dict.
class _labeldata(ctypes.Structure):
    _fields_ = [("name", c_char_p), ("location", c_int)]
...
class writtenblockdata(ctypes.Structure):
    _fields_ = [("pcoffset", c_int), ("snesoffset", c_int), ("numbytes", c_int)]

    def __repr__(self) -> str:
        return f"<written block ${self.snesoffset:06x} 0x{self.pcoffset:x} size:{self.numbytes}>"


# internal use only. patch() accepts a dict.
class _memoryfile(ctypes.Structure):
    _fields_ = [("path", c_char_p), ("buffer", c_char_p), ("length", ctypes.c_size_t)]


# internal use only. patch() accepts a dict.
class _warnsetting(ctypes.Structure):
    _fields_ = [("warnid", c_char_p), ("enabled", ctypes.c_bool)]
...
class _AsarDLL:
    def __init__(self, dllname) -> None:
        ...
        try:
            ...
            self.setup_func("patch", (c_char_p, c_char_p, c_int, c_int_ptr), ctypes.c_bool)
            ...
            self.setup_func("math", (c_char_p, POINTER(c_char_p)), ctypes.c_double)
            self.setup_func("getwrittenblocks", (c_int_ptr,), POINTER(writtenblockdata))
            self.setup_func("getmapper", (), c_int)
            self.setup_func("getsymbolsfile", (c_char_p,), c_char_p)

        except AttributeError:
            msg = "Asar DLL is missing some functions"
            raise OSError(msg)
        api_ver = dll.asar_apiversion()
        if api_ver < _target_api_ver or (api_ver // 100) > (_target_api_ver // 100):
    ...
    def setup_func(
        ...
        argtypes: tuple[()]
        | tuple[type[c_char_p]]
        | tuple[type[c_char_p], type[c_char_p], type[c_int], type[_Pointer[c_int]]]
        | tuple[type[c_char_p], type[_Pointer[c_char_p]]]
        | tuple[type[_Pointer[_patchparams]]]
        | tuple[type[_Pointer[c_int]]],
        restype: type[c_bool]
        | type[c_char_p]
        | type[c_double]
        | type[c_int]
        | type[_Pointer[_definedata]]
        | type[_Pointer[_labeldata]]
        | type[_Pointer[c_char_p]]
        | type[_Pointer[errordata]]
        | type[_Pointer[writtenblockdata]]
        | None,
    ) -> None:
...
def init(dll_path=None) -> bool | None:
    """Load the Asar DLL.
...
def patch(
    ...
    additional_defines=None,
    std_include_file=None,
    std_define_file=None,
...
def resolvedefines(data, learnnew: bool = False):
...
def getsymbolsfile(fmt: str = "wla"):
