...
ASCII_TO_INT: dict[bytes, int] = {i.to_bytes(1, "big"): i for i in range(256)}
INT_TO_ASCII: dict[int, bytes] = {i: b for b, i in ASCII_TO_INT.items()}
MAX_KEY_LEN = 512
...
def compress(data: bytes | str) -> bytes:
    ...
    while True:
        ...
        for i in range(1, n_data - start):
            ...
            break
    bits: str = "".join([f"{i:b}".zfill(9) for i in compressed])
...
def decompress(data: bytes | str) -> bytes:
    ...
    bits: str = f"{int.from_bytes(data, "big"):b}".zfill(len(data) * 8)
